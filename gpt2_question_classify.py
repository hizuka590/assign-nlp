...
class QuestionClassify(object):
    def __init__(self):
        ...
        self.train_file = os.path.join(cur, 'data/qa_corpus.txt')
        self.embedding_path = os.path.join(cur, 'CAIL2018_ALL_DATA/word_vec_300.bin')
        ...
        self.tbCallBack_cnn = TensorBoard(log_dir="logs/fit/" +"cnn_"+ datetime.datetime.now().strftime("%Y%m%d-%H%M%S"),  # log 目录
        ...
        self.tbCallBack_lstm = TensorBoard(log_dir="logs/fit/" +"cnn_"+ datetime.datetime.now().strftime("%Y%m%d-%H%M%S"),  # log 目录
                                      histogram_freq=0,  # 按照何等频率（epoch）来计算直方图，0为不计算
                                      #                  batch_size=32,     # 用多大量的数据计算直方图
                                      write_graph=True,  # 是否存储网络结构图
                                      write_grads=True,  # 是否可视化梯度直方图
    ...
    def train_cnn(self):
        ...
        model.fit(X_train, Y_train, batch_size=100, epochs=50, validation_data=(X_test, Y_test),callbacks=[self.tbCallBack_cnn])
    ...
    def train_lstm(self):
        ...
        model.fit(X_train, Y_train, batch_size=100, epochs=50, validation_data=(X_test, Y_test),callbacks=[self.tbCallBack_lstm])
